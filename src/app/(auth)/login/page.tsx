...
export default function LoginPage() {
 ...
 const supabase = createClient();
  ...
  />
        ...
        <h1 className="text-center text-white/90 text-xl font-semibold mb-2">Welcome back</h1>
        ...
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          {err && <p className="mt-2 text-center text-red-200 text-sm">{err}</p>}
        </form>

        <div className="flex items-center justify-between mt-4 text-sm">
          <Link href="/reset" className="text-white/80 hover:text-white">Forgot password?</Link>
          <Link href="/signup" className="text-white/80 hover:text-white">Create account</Link>
        </div>
      </GlassPanel>
    </div>
  );
