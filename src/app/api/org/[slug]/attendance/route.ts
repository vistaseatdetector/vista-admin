...
export async function GET(_: Request, { params }: { params: { slug: string } }) {
  ...
  const { data: org, error: oErr } = await supabase
    .from("orgs").select("id, name").eq("slug", params.slug).maybeSingle();
  ...
  const rows = (data ?? []).map(r =>
    `${new Date(r.observed_at as string).toISOString()},${nameById.get(r.location_id as any) ?? ""},${r.people_count},${r.open_seats ?? ""}`
  );
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${params.slug}-attendance-24h.csv"`
    }
  });
}
