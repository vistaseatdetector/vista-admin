  ...
  return (
    ...
    >
      {/* Shared video component for consistent streams */}
        <SharedCamera
          key={featuredStreamId}
          cameraSource={featuredStreamId}
          enableDetection={true}
          detectionFrameRate={10}
          onDetection={(result) => {
...
export default function DoorsPage() {
  ...
  const [streams, setStreams] = useState<Array<{
    ...
    kind: string;
    url?: string;
    is_active: boolean; // kept for backward compat in UI only
  }>>([]);
  const [isLoading, setIsLoading] = useState(true);

  // Featured stream: prefer the selected door's camera_id (url), else pick first webcam
  const selectedDoor = selectedDoorId ? doors.find(d => d.id === selectedDoorId) : undefined;
  const defaultFeaturedStream = streams.find(s => s.url?.startsWith('webcam:') || s.kind === 'webcam') || streams[0];
  const featuredStreamId = (selectedDoor?.camera_id && typeof selectedDoor.camera_id === 'string' ? selectedDoor.camera_id : undefined)
    || (defaultFeaturedStream?.url || 'webcam:0');
  ...
  const handleDeleteDoor = async (doorId: string) => {
    ...
    try {
      const resp = await fetch(`/api/org/${slugParam}/doors`, {
        method: 'DELETE',
  ...
  // Load door zones (local) and doors (server)
  useEffect(() => {
    ...
    // Load doors from API for this org
    (async () => {
      try {
        const resp = await fetch(`/api/org/${slugParam}/doors`);
        if (resp.ok) {
          ...
          setDoors(data.doors || []);
          localStorage.setItem('doors', JSON.stringify(data.doors || []));
        }
      } catch (e) {
        console.error('❌ Failed to load doors from server:', e);
      }
    })();
  }, [selectedDoorId, slugParam]);
  ...
  // Save door zones to localStorage and send to backend
  const saveDoorZones = async () => {
    ...
    // Persist zones to Supabase for this org+stream (display-space coords)
    try {
      const slugParam = Array.isArray(params.slug) ? (params.slug as string[])[0] : (params.slug as string);
    ...
    // Send zones to backend for tracking
    try {
      console.log('🎯 Sending zones to backend:', doorSpecificZones);
      
      // Convert display-space zone coordinates to frame-space
      ...
      // Merge with existing zones for other doors and save
      ...
      console.log('✅ Zones updated on backend:', result);
      
      const selectedDoor = doors.find(d => d.id === selectedDoorId);
      alert(`Door zones saved successfully for "${selectedDoor?.name}"! Zone-based counting is now active.`);
    } catch (error) {
      console.error('❌ Failed to update zones on backend:', error);
      const selectedDoor = doors.find(d => d.id === selectedDoorId);
      alert(`Door zones saved locally for "${selectedDoor?.name}", but backend update failed. Detection may not work properly.`);
    }
  };

  // Load zones from Supabase for this stream
  ...
  const handleAddCamera = async () => {
    ...
    // Use the stream's URL (e.g., 'webcam:X') for door camera_id
    const cameraSource = selectedCamera.url || '';
    if (!cameraSource) {
      alert('Selected camera has no configured URL. Please set its URL first.');
      return;
    }
